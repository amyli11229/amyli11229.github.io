---
# ============================================================
# EDIT THIS PROJECT — then run:  npm run sync
# Full guide: content/projects/README.md
# ============================================================

# --- Show / hide / order ---
published: true
sort_order: 3

# --- Project page ---
title: Dig! Dig! Dig!
organization: Brackeys Game Jam 2025.2
tags: Game Design, Pixel Art, Procedural Generation
lead: Design lead on a four-person Brackeys Game Jam team—pixel art direction and player-facing experience for a short procedural digging game.
hero_image: images/projects/dig-dig-dig.png

# --- Preview card (homepage + Projects list) ---
card_year: Game Jam Project
card_description: Procedural pixel-art digging game for Brackeys Game Jam—visual direction for a four-person team.
card_medium: Game Jam / Pixel Art

# --- Optional: video ---
project_video:
  title: Dig! Dig! Dig! Demo
  youtube_id: LkRx0d3vdBQ

# --- Optional: download / visit button ---
prototype_callout:
  title: Download
  text: Play the Brackeys Game Jam build on itch.io.
  button_label: Download on itch.io
  button_url: https://squiffy121.itch.io/dig-dig-dig

# --- Pixel art gallery ---
additional_images_heading: Pixel art
additional_images_intro: Environment layers, drill variants, loot, and UI assets from the game’s visual direction.
additional_images_pixel: true
additional_images:
  - image: images/projects/dig-dig-dig/parallax-cave-3.png
    caption: Parallax cave — rock openings
    wide: true
  - image: images/projects/dig-dig-dig/drill-pink.png
    caption: Pink drill — thrust frames
  - image: images/projects/dig-dig-dig/drill-pink-alt.png
    caption: Pink drill — animation sheet
  - image: images/projects/dig-dig-dig/drill-purple.png
    caption: Purple drill — animation sheet
  - image: images/projects/dig-dig-dig/drill-gold.png
    caption: Gold drill — animation sheet
  - image: images/projects/dig-dig-dig/ore-crystal.png
    caption: Crystal ore collectible
    large: true
  - image: images/projects/dig-dig-dig/loot-gold.png
    caption: Gold loot pile
    large: true
  - image: images/projects/dig-dig-dig/ui-hearts.png
    caption: Health UI — full / empty

# --- My Approach ---
contributions:
  - Led design for a 4-person game jam team
  - Defined the pixel art visual direction and overall look of the game
  - Shaped player-facing experience around digging, combat, and risk-reward decisions
  - Collaborated with teammates on gameplay feel and presentation for a jam submission

# --- Optional: extra links ---
external_links:
  - label: Play Dig! Dig! Dig! on itch.io
    url: https://squiffy121.itch.io/dig-dig-dig
  - label: Watch the demo video
    url: https://www.youtube.com/watch?v=LkRx0d3vdBQ

# --- Usually leave alone ---
slug: dig-dig-dig
layout: layouts/project-detail.njk
---

Players dig for loot under a 120-second timer, then race rising lava. Built with Squiffy121, Nng11, and jamesedra for Brackeys Game Jam 2025.2.
